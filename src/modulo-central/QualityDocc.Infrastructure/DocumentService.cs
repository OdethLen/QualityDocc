        ...
        public async Task<List<DocumentDto>> GetAllDocumentsAsync()
        {
            return await _context.Document
                ...
                .Select(d => new DocumentDto
                {
                    ...
                    VersionNumber = d.Versions.OrderByDescending(v => v.VersionNumber)
                                              .FirstOrDefault().VersionNumber.ToString(),
                    CurrentStatus = d.CurrentStatus,
                    ChangeDate = d.Versions.OrderByDescending(v => v.DateCreate)
                                           .FirstOrDefault().DateCreate,
                    CreatedBy = "Usuario" // O cámbialo por d.Author.Name si tienes la relación
                })
                .ToListAsync();
        }
